...
parser.add_argument("--api-key", type=str, default=None, help="Require authentication via API key for all control-plane operations. Clients must send the key in the Authorization header as 'Bearer <key>'. When set, unauthenticated requests to workflow submission, queue control, and other sensitive endpoints will be rejected with 401.")
